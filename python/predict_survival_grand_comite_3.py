...
RÔLE   : Charger DenseNet, ResNet ET le modèle F1 du "Petit Poucet" pour
         un vote à 3 voix sur les cubes segmentés.
=============================================================================
"""
...
def run_grand_comite_prediction():
    print("⏳ Initialisation du Grand Comité 3 modèles (Dense, Res, F1)...")
    ...
    # 3. Chargement du modèle F1 du Petit Poucet
    model_f1 = create_alternative_survival_classifier(spatial_dims=3, in_channels=1, num_classes=3).to(device)
    model_f1.load_state_dict(torch.load(WEIGHTS_DIR / "best_petit_poucet_fold_1.pt", map_location=device, weights_only=True))
    model_f1.eval()
    
    print("✅ Les 3 modèles (Dense, Res, F1) sont déployés et prêts !\n")
    ...
    # En-tête de tableau pour les 3 modèles
    print(f"{'Patient':<25}| {'Dense':<5} | {'Res':<3} | {'F1':<2}  || {'Vote IA':<8} | {'Vrai':<8} | {'Résultat':<10}")
    print("-" * 90)
        ...
        for cube_path in cube_files:
            ...
            try:
                ...
                with torch.amp.autocast('cuda'):
                    ...
                    pred_f1 = torch.argmax(model_f1(input_tensor), dim=1).item()
                
                # Vote des 3 modèles
                tous_les_votes = [pred_dense, pred_res, pred_f1]
                vote_majoritaire = Counter(tous_les_votes).most_common(1)[0][0]
                
                c_dense = mapping[pred_dense][0]
                c_res = mapping[pred_res][0]
                c_f1 = mapping[pred_f1][0]
                ...
                print(f"ID {patient_name[:25]:<25} | {c_dense:<5} | {c_res:<3} | {c_f1:<2}  || {txt_final:<8} | {txt_vrai:<8} | {statut}")
